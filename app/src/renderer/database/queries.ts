...
export const getAndCreateIfNotExistsTodoList = async (
  date: TDateISODate
): Promise<TTodoList> => {
...
export const upsertTodoList = async (
  date: TDateISODate,
  taskIds: string[] = []
) => {
  const existingTodoList = await database.todoList
    .where("date")
    .equals(date)
    .first();
...
export const removeTaskFromTodoList = async (
  date: TDateISODate,
  taskId: string
) => {
...
export const getPreviousDayActiveTasks = async (date: TDateISODate) => {
  const lastSelectedDate = (await database.todoList.toArray())
    .filter((entry) => entry.date < date)
    .reverse()[0];
  if (!lastSelectedDate) return;

  const previousDayTasks = await database.todoList
    .where({ date: lastSelectedDate.date })
    .first();
  const activeTasks = await getActiveTasks();

  const previousDayActiveTasks =
    previousDayTasks?.taskIds.filter((id) => activeTasks[id]) ?? [];
...
export const updateSubtask = async (
  taskId: string,
  subtaskId: string,
  subtaskUpdate: Partial<TSubtask>
) => {
  const task = await database.tasks.where("id").equals(taskId).first();
  await database.tasks
    .where("id")
